...
def print_AMS(y = None, weights = None, n_sample = 5, plot_type = "all", variable_type = "absolute", max_AMS = 10. ):
    ...
    variable_type : string in ["percent", "absolute"]
    ...
    if variable_type == "percent": 
        sum_1 = np.sum(y)
        sum_0 = y.shape[0] - sum_1

    def get_AMS_estimation_percent(s_p, b_p):
        """
    ...
    def get_AMS_estimation_absolute(s, b):
        """
        s = s final (apres pondération pour etre ramene a la taille 25000)
        b = b final (apres pondération pour etre ramene a la taille 25000)
        """
        return hbc.AMS(s,b)
    ...
    if variable_type == "percent":
        ...
        for i in range(20):
            for j in range(20):
                Z[i,j] = get_AMS_estimation_percent(s_p[i], b_p[j])
                if Z[i,j] >  max_AMS:
                    Z[i,j] = max_AMS
    ...
    if variable_type == "absolute":
        ...
        for i in range(100):
            for j in range(100):
                Z[i,j] = get_AMS_estimation_absolute(s[i], b[j])
                if Z[i,j] > max_AMS:
                    Z[i,j] = max_AMS
